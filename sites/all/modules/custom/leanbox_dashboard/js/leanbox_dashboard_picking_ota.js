...
(function ($) {
	...
	Drupal.behaviors.leanbox_dashboard_picking_ota = {
		attach: function (context, settings) {

			var area_definition = Drupal.settings.leanbox_dashboard.area_definition;
			var party_pack_data = Drupal.settings.leanbox_dashboard.picking_ota;
			google.charts.load("current", {packages: ['gauge', 'corechart', 'bar']});
			google.charts.setOnLoadCallback(function () {
				drawpartypackingChart(party_pack_data);
			});

			$(".picking-ota-submit").click(function (e) {
				...
				if (start_date != '' && end_date != '') {
					$.ajax({
						...
						data: {start_date: start_date, end_date: end_date, activity_type: 'picking_ota'},
