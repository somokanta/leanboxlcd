...
(function ($) {
	...
	Drupal.behaviors.leanbox_dashboard_bills_per_person = {
		attach: function (context, settings) {
			
			var area_definition = Drupal.settings.leanbox_dashboard.area_definition;
			var party_pack_data = Drupal.settings.leanbox_dashboard.bills_per_person;
			//google.charts.load("current", {packages: ['gauge','corechart', 'bar']});
			google.charts.setOnLoadCallback(function () {
				drawpartypackingChart(party_pack_data);
			});

			$(".bills-person-submit").click(function (e) {
			  ...
			  var start_date = $(this).parent().prev().prev().find("input[name='start_date[date]']").val();
				...
				if (start_date != '' && end_date != '') {
					$.ajax({
						...
						data: {start_date: start_date, end_date: end_date, activity_type: 'bills_per_person'},
	      ...
	      var data = google.visualization.arrayToDataTable(party_pack_data);
				...
				var options = {
					...
					vAxis: {
						minValue: 0,
						maxValue: 8,
						title: 'Bills per person'
					},
					hAxis: {
						title: 'Supervisors'
					},
				};
				var chart = new google.visualization.ColumnChart(document.getElementById("bills_per_person_div"));
